...
class SmoothQuantTuner(Algorithm):
    ...
    def apply(
        ...
        nodes_to_tune = list(sq_statistic_points.data.keys())

        for node_name in nodes_to_tune:
            scale_by_node_name[node_name] = {"diff": np.inf, "scale": 0}
        fp_container = self.get_container_for_nodes(
            nodes_to_tune, self._algorithm_key, output_port_id, self._subset_size
        )
        ...
        nncf_logger.info(self._scales_grid)

        for scale in self._scales_grid:
            nncf_logger.info(f"Tuning with {scale} scale")
            modified_model = deepcopy(model)
            self._smooth_quant._alpha_map["matmul"] = scale
            ...
            nncf_logger.info(">>> Collecting INT statistics for MinMax")
            ...
            for node_name in nodes_to_tune:
                ...
                fp_statistics = fns.stack(fp_statistics)
                int_statistics = self.get_statistics_for_node(
                    int_post_layer_statistics, node_name, self._algorithm_key, output_port_id
                )
                int_statistics = fns.stack(int_statistics)

                diff = fns.mean(fns.abs(fp_statistics - int_statistics)).data

                if diff < scale_by_node_name[node_name]["diff"]:
                    scale_by_node_name[node_name] = {"diff": diff, "scale": scale}
            del int_post_layer_statistics

        nncf_logger.info(">>> Final quantization")
        self._smooth_quant._alpha_map_by_layers = {n: d["scale"] for n, d in scale_by_node_name.items()}
    ...
    def get_statistics_for_node(statistic_points, node_name, key, output_port_id):
        ...
        for tensor_collector in statistic_points.get_algo_statistics_for_node(
            ...
            statistic = tensor_collector.get_statistics().mean_values
    ...
    def get_container_for_nodes(node_names, key, output_port_id, subset_size):
        ...
        for node_name in node_names:
            ...
            stat_collector = get_mean_statistic_collector(subset_size, -1)

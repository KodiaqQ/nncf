...
@staticmethod
def build_for_fast_bc(model: ov.Model, node: NNCFNode, act_port_id: int, weight_port_id: int, out_port_id: int = 0) -> ov.Model:
    """
    ...
    if node.metatype == om.OVConvolutionMetatype:
        main_node_params.update(
            {"data": parameter, "filters": weights_fq, "name": original_node.get_friendly_name()}
        )
        main_node = opset.convolution(**main_node_params)
    elif node.metatype == om.OVMatMulMetatype:
        main_node_params.update(
            {"data_a": parameter, "data_b": weights_fq, "name": original_node.get_friendly_name()}
        )
